...
Nasa9Poly1::Nasa9Poly1(CanteraDouble tlow, CanteraDouble thigh, CanteraDouble pref,
...
void Nasa9Poly1::setParameters(const vector<CanteraDouble> &coeffs)
{
    if (coeffs.size() != 9) {
...
void Nasa9Poly1::updateTemperaturePoly(CanteraDouble T, CanteraDouble* T_poly) const
{
    T_poly[0] = T;
    T_poly[1] = T * T;
    T_poly[2] = T_poly[1] * T;
...
void Nasa9Poly1::updateProperties(const CanteraDouble* tt, CanteraDouble* cp_R, CanteraDouble* h_RT,
                                  CanteraDouble* s_R) const
{

    CanteraDouble ct0 = m_coeff[0] * tt[5]; // a0 / (T^2)
    CanteraDouble ct1 = m_coeff[1] * tt[4]; // a1 / T
    CanteraDouble ct2 = m_coeff[2]; // a2
    CanteraDouble ct3 = m_coeff[3] * tt[0]; // a3 * T
    CanteraDouble ct4 = m_coeff[4] * tt[1]; // a4 * T^2
    CanteraDouble ct5 = m_coeff[5] * tt[2]; // a5 * T^3
    CanteraDouble ct6 = m_coeff[6] * tt[3]; // a6 * T^4

    CanteraDouble cpdivR = ct0 + ct1 + ct2 + ct3 + ct4 + ct5 + ct6;
    CanteraDouble hdivRT = -ct0 + tt[6]*ct1 + ct2 + 0.5*ct3 + 1.0/3.0*ct4
                        + 0.25*ct5 + 0.2*ct6 + m_coeff[7] * tt[4];
    CanteraDouble sdivR = -0.5*ct0 - ct1 + tt[6]*ct2 + ct3 + 0.5*ct4
...
void Nasa9Poly1::updatePropertiesTemp(const CanteraDouble temp, CanteraDouble* cp_R, CanteraDouble* h_RT,
                                      CanteraDouble* s_R) const
{
    CanteraDouble tPoly[7];
    updateTemperaturePoly(temp, tPoly);
    updateProperties(tPoly, cp_R, h_RT, s_R);
}

void Nasa9Poly1::reportParameters(size_t& n, int& type, CanteraDouble& tlow, CanteraDouble& thigh,
