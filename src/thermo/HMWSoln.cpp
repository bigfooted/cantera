...
CanteraDouble HMWSoln::enthalpy_mole() const
{
...
{
    getPartialMolarEnthalpies(m_tmpV.data());
    CanteraDouble hbar = mean_X(m_tmpV);
...
CanteraDouble HMWSoln::entropy_mole() const
{
    getPartialMolarEntropies(m_tmpV.data());
    return mean_X(m_tmpV);
}

CanteraDouble HMWSoln::gibbs_mole() const
{
    getChemPotentials(m_tmpV.data());
    return mean_X(m_tmpV);
}

CanteraDouble HMWSoln::cp_mole() const
{
...
void HMWSoln::calcDensity()
{
    ...
    CanteraDouble dd = meanMolecularWeight() / mean_X(m_tmpV);
...
CanteraDouble HMWSoln::standardConcentration(size_t k) const
{
    ...
    CanteraDouble mvSolvent = m_tmpV[0];
    if (k > 0) {
    ...
    if (m_input.hasKey("activity-data")) {
        ...
        if (actData.hasKey("interactions")) {
            for (auto& item : actData["interactions"].asVector<AnyMap>()) {
                ...
                CanteraDouble q0 = charge(speciesIndex(species[0]));
                CanteraDouble q1 = (nsp > 1) ? charge(speciesIndex(species[1])) : 0;
                CanteraDouble q2 = (nsp == 3) ? charge(speciesIndex(species[2])) : 0;
                if (nsp == 2 && q0 * q1 < 0) {
...
{
    calcMolalities();
    vector<CanteraDouble>& moleF = m_tmpV;
