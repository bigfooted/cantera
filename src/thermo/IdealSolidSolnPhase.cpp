...
CanteraDouble IdealSolidSolnPhase::enthalpy_mole() const
{
    CanteraDouble htp = RT() * mean_X(enthalpy_RT_ref());
...
void IdealSolidSolnPhase::getChemPotentials_RT(CanteraDouble* mu) const
{
    warn_deprecated("IdealSolidSolnPhase::getChemPotentials_RT",
                    "To be removed after Cantera 3.0. Use getChemPotentials instead.");
    CanteraDouble delta_pdRT = (m_Pcurrent - m_Pref) / (temperature() * GasConstant);
    const vector<CanteraDouble>& g_RT = gibbs_RT_ref();
    for (size_t k = 0; k < m_kk; k++) {
        CanteraDouble xx = std::max(SmallNumber, moleFraction(k));
        mu[k] = (g_RT[k] + log(xx))
...
void IdealSolidSolnPhase::getPureGibbs(CanteraDouble* gpure) const
{
