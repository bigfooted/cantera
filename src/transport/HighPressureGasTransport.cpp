...
CanteraDouble HighPressureGasTransport::thermalConductivity()
{
    ...
    update_T();
    CanteraDouble Lprime_m = 0.0;
    const CanteraDouble c1 = 1./16.04;
    size_t nsp = m_thermo->nSpecies();
    vector<CanteraDouble> molefracs(nsp);
    m_thermo->getMoleFractions(&molefracs[0]);
    vector<CanteraDouble> cp_0_R(nsp);
    m_thermo->getCp_R_ref(&cp_0_R[0]);

    vector<CanteraDouble> L_i(nsp);
    vector<CanteraDouble> f_i(nsp);
    vector<CanteraDouble> h_i(nsp);
    vector<CanteraDouble> V_k(nsp);

    m_thermo -> getPartialMolarVolumes(&V_k[0]);
    CanteraDouble L_i_min = BigNumber;

    for (size_t i = 0; i < m_nsp; i++) {
        CanteraDouble Tc_i = Tcrit_i(i);
        CanteraDouble Vc_i = Vcrit_i(i);
        CanteraDouble T_r = m_thermo->temperature()/Tc_i;
        CanteraDouble V_r = V_k[i]/Vc_i;
        CanteraDouble T_p = std::min(T_r,2.0);
        CanteraDouble V_p = std::max(0.5,std::min(V_r,2.0));

        // Calculate variables for density-independent component:
        CanteraDouble theta_p = 1.0 + (m_w_ac[i] - 0.011)*(0.56553
            - 0.86276*log(T_p) - 0.69852/T_p);
        CanteraDouble phi_p = (1.0 + (m_w_ac[i] - 0.011)*(0.38560
            - 1.1617*log(T_p)))*0.288/Zcrit_i(i);
        CanteraDouble f_fac = Tc_i*theta_p/190.4;
        CanteraDouble h_fac = 1000*Vc_i*phi_p/99.2;
        CanteraDouble T_0 = m_temp/f_fac;
        CanteraDouble mu_0 = 1e-7*(2.90774e6/T_0 - 3.31287e6*pow(T_0,-2./3.)
        ...
        CanteraDouble H = sqrt(f_fac*16.04/m_mw[i])*pow(h_fac,-2./3.);
        CanteraDouble mu_i = mu_0*H*m_mw[i]*c1;
        ...
        CanteraDouble theta_s = 1 + (m_w_ac[i] - 0.011)*(0.09057 - 0.86276*log(T_p)
            + (0.31664 - 0.46568/T_p)*(V_p - 0.5));
        CanteraDouble phi_s = (1 + (m_w_ac[i] - 0.011)*(0.39490*(V_p - 1.02355)
    ...
    CanteraDouble h_m = 0;
    CanteraDouble f_m = 0;
    CanteraDouble mw_m = 0;
    for (size_t i = 0; i < m_nsp; i++) {
        for (size_t j = 0; j < m_nsp; j++) {
            // Density-independent component:
            CanteraDouble L_ij = 2*L_i[i]*L_i[j]/(L_i[i] + L_i[j] + Tiny);
            ...
            CanteraDouble h_ij = 0.125*pow(pow(h_i[i],1./3.) + pow(h_i[j],1./3.),3.);
            CanteraDouble mw_ij_inv = (m_mw[i] + m_mw[j])/(2*m_mw[i]*m_mw[j]);
    ...
    CanteraDouble rho_0 = 16.04*h_m/(1000*m_thermo->molarVolume());
    ...
    CanteraDouble L_1m = 1944*mu_0;
    CanteraDouble L_2m = (-2.5276e-4 + 3.3433e-4*pow(1.12 - log(T_0/1.680e2),2))*rho_0;
    CanteraDouble L_3m = exp(-7.19771 + 85.67822/T_0)*(exp((12.47183
                - 984.6252*pow(T_0,-1.5))*pow(rho_0,0.1) + (rho_0/0.1617 - 1)
                *sqrt(rho_0)*(0.3594685 + 69.79841/T_0 - 872.8833*pow(T_0,-2))) - 1.)*1e-3;
    CanteraDouble H_m = sqrt(f_m*16.04/mw_m)*pow(h_m,-2./3.);
    CanteraDouble Lstar_m = H_m*(L_1m + L_2m + L_3m);
    return Lprime_m + Lstar_m;
}

void HighPressureGasTransport::getThermalDiffCoeffs(CanteraDouble* const dt)
{
...
void HighPressureGasTransport::getBinaryDiffCoeffs(const size_t ld, CanteraDouble* const d)
{
    vector<CanteraDouble> PcP(5);
    size_t nsp = m_thermo->nSpecies();
    vector<CanteraDouble> molefracs(nsp);
    m_thermo->getMoleFractions(&molefracs[0]);
    ...
    for (size_t i = 0; i < nsp; i++) {
        for (size_t j = 0; j < nsp; j++) {
            ...
            CanteraDouble x_i = std::max(Tiny, molefracs[i]);
            CanteraDouble x_j = std::max(Tiny, molefracs[j]);
...
void HighPressureGasTransport::getMultiDiffCoeffs(const size_t ld, CanteraDouble* const d)
{
...
CanteraDouble HighPressureGasTransport::viscosity()
{
    ...
    CanteraDouble Tc_mix = 0.;
    CanteraDouble Pc_mix_n = 0.;
    CanteraDouble Pc_mix_d = 0.;
    CanteraDouble MW_mix = m_thermo->meanMolecularWeight();
    CanteraDouble MW_H = m_mw[0];
    CanteraDouble MW_L = m_mw[0];
    CanteraDouble FP_mix_o = 0;
    CanteraDouble FQ_mix_o = 0;
    CanteraDouble tKelvin = m_thermo->temperature();
    CanteraDouble Pvp_mix = m_thermo->satPressure(tKelvin);
    size_t nsp = m_thermo->nSpecies();
    vector<CanteraDouble> molefracs(nsp);
    m_thermo->getMoleFractions(&molefracs[0]);

    CanteraDouble x_H = molefracs[0];
    for (size_t i = 0; i < m_nsp; i++) {
        ...
        CanteraDouble mu_ri = 52.46*100000*m_dipole(i,i)*m_dipole(i,i)
            *Pcrit_i(i)/(Tc*Tc);
        if (mu_ri < 0.022) {
    ...
    CanteraDouble ksi = pow(GasConstant*Tc_mix*3.6277*pow(10.0,53.0)/(pow(MW_mix,3)
    ...
    // Calculate Z1m
    CanteraDouble Z1m = (0.807*pow(Tr_mix,0.618) - 0.357*exp(-0.449*Tr_mix)
    ...
    if (Tr_mix <= 1.0) {
        if (Pr_mix < Pvp_mix/Pc_mix) {
            CanteraDouble alpha = 3.262 + 14.98*pow(Pr_mix,5.508);
            CanteraDouble beta = 1.390 + 5.746*Pr_mix;
        ...
        if ((Pr_mix > 0.0) && (Pr_mix <= 100.0)) {
            CanteraDouble a_fac = 0.001245*exp(5.1726*pow(Tr_mix,-0.3286))/Tr_mix;
            CanteraDouble b_fac = a_fac*(1.6553*Tr_mix - 1.2723);
            CanteraDouble c_fac = 0.4489*exp(3.0578*pow(Tr_mix,-37.7332))/Tr_mix;
            CanteraDouble d_fac = 1.7368*exp(2.2310*pow(Tr_mix,-7.6351))/Tr_mix;
            CanteraDouble f_fac = 0.9425*exp(-0.1853*pow(Tr_mix,0.4489));
...
CanteraDouble HighPressureGasTransport::Tcrit_i(size_t i)
{
    // Store current molefracs and set temp molefrac of species i to 1.0:
    vector<CanteraDouble> molefracs = store(i, m_thermo->nSpecies());

    CanteraDouble tc = m_thermo->critTemperature();
    // Restore actual molefracs:
    m_thermo->setMoleFractions(&molefracs[0]);
    return tc;
}

CanteraDouble HighPressureGasTransport::Pcrit_i(size_t i)
{
    // Store current molefracs and set temp molefrac of species i to 1.0:
    vector<CanteraDouble> molefracs = store(i, m_thermo->nSpecies());

    CanteraDouble pc = m_thermo->critPressure();
    // Restore actual molefracs:
    m_thermo->setMoleFractions(&molefracs[0]);
    return pc;
}

CanteraDouble HighPressureGasTransport::Vcrit_i(size_t i)
{
    // Store current molefracs and set temp molefrac of species i to 1.0:
    vector<CanteraDouble> molefracs = store(i, m_thermo->nSpecies());

    CanteraDouble vc = m_thermo->critVolume();
    // Restore actual molefracs:
    m_thermo->setMoleFractions(&molefracs[0]);
    return vc;
}

CanteraDouble HighPressureGasTransport::Zcrit_i(size_t i)
{
    // Store current molefracs and set temp molefrac of species i to 1.0:
    vector<CanteraDouble> molefracs = store(i, m_thermo->nSpecies());

    CanteraDouble zc = m_thermo->critCompressibility();
    // Restore actual molefracs:
    m_thermo->setMoleFractions(&molefracs[0]);
    return zc;
}

vector<CanteraDouble> HighPressureGasTransport::store(size_t i, size_t nsp)
{
    vector<CanteraDouble> molefracs(nsp);
    m_thermo->getMoleFractions(&molefracs[0]);
    vector<CanteraDouble> mf_temp(nsp, 0.0);
    mf_temp[i] = 1;
...
CanteraDouble HighPressureGasTransport::FQ_i(CanteraDouble Q, CanteraDouble Tr, CanteraDouble MW)
{
...
CanteraDouble HighPressureGasTransport::setPcorr(CanteraDouble Pr, CanteraDouble Tr)
{
    ...
    CanteraDouble P_corr_1 = DP_Rt_lookup[Pr_i]*(1.0 - A_ij_lookup[Pr_i]
        *pow(Tr,-B_ij_lookup[Pr_i]))*(1-C_ij_lookup[Pr_i]
        *pow(Tr,-E_ij_lookup[Pr_i]));
    CanteraDouble P_corr_2 = DP_Rt_lookup[Pr_i+1]*(1.0 - A_ij_lookup[Pr_i+1]
